...
std::wstring _to_wstring(const std::string &str, int code_page = CP_ACP) {
    ...
    MultiByteToWideChar(code_page, 0, str.c_str(), (int)str.size(), buffer,
                        len);
    buffer[len] = '\0';
    res.append(buffer);
    delete[] buffer;
...
std::string _to_string(const std::wstring &wstr, int code_page = CP_ACP) {
    ...
    WideCharToMultiByte(code_page, 0, wstr.c_str(), (int)wstr.size(), buffer,
                        len, NULL, NULL);
    buffer[len] = '\0';
    res.append(buffer);
    delete[] buffer;
...
void list_files_to_vector(const Path &directory, StrVec &files,
...
int transfer_progress(const git_transfer_progress *stats, void *payload) {
    ...
    float percentage = static_cast<float>(stats->received_objects) /
    ...
    if (static_cast<int>(percentage) != last_percent) {
        ...
        std::cout << "] " << stats->received_objects << "/"
                  << stats->total_objects << " objs " << int(percentage)
                  << " %\r";
        std::cout.flush();
    }
    return 0;
...
int update_repository(const char *repo_path, const char *proxy_opts) {
    ...
    error = git_branch_lookup(&origin_ref, repo, origin_branch_name.c_str(),
                              GIT_BRANCH_REMOTE);
    if (error) {
        std::cout << "error git_branch_lookup " << origin_branch_name
                  << std::endl;
        FINALIZE_GIT(error);
    }
    ...
    git_oid local_oid;
    git_oid remote_oid;
    error = git_reference_name_to_id(&local_oid, repo,
                                     git_reference_name(local_ref));
    ...
    if (!std::equal(local_oid.id, local_oid.id + sizeof(local_oid),
        ...
        if (error == 1) {
            ...
            error = git_reference_set_target(
                &updated_ref, local_ref, git_annotated_commit_id(origin_commit),
                "Fast-forward");
        } else if (error < 0) {
...
// recipes processes
// ----------------------------------------------------------------------------
int install_recipe_impl(const std::optional<StrVec> &recipes,
                        const std::string &prompt, json &installed_recipes) {
    if (recipes != std::nullopt) {
        for (const auto &dep : recipes.value()) {
            ...
            int error = clone_or_update_repository(
                repo_url.c_str(), local_path.c_str(), proxy.c_str());
            if (error)
                return error;
            StrVec files;
...
// install recipe repo
int install_recipe(const spec::Recipe &recipe,
    ...
    if (error)
        return error;
    error =
        install_recipe_impl(recipe.reverse_dependencies,
...
void delete_recipe_impl(const std::optional<StrVec> &recipes,
                        const std::string &prompt, json &installed_recipes) {
    if (recipes != std::nullopt) {
        for (const auto &dep : recipes.value()) {
            std::string repo_url = mirror + dep + ".git";
...
// delete recipe repo
int delete_recipe(const spec::Recipe &recipe,
                  const std::string &recipe_file = "", bool purge = false) {
    // load cache_dir/.installed_recipes.json
    json installed_recipes = jsonutils::load_from_file(installed_recipes_json);
    StrVec recipes;
    recipes.emplace_back(recipe.repo);
    ...
    delete_recipe_impl(recipe.dependencies,
                       "update dependency: ", installed_recipes);
    delete_recipe_impl(recipe.reverse_dependencies,
                       "update reverse dependency: ", installed_recipes);
    jsonutils::save_to_file(installed_recipes, installed_recipes_json);
    return 0;
}
...
std::string join_string_vector(const StrVec &string_vector,
...
int main(int argc, char **argv) {
    ...
    try {
        ...
        options.add_options()("h,help", "print help")(
            ...
            "P,purge",
            "purge a recipe (with dependencies and reverseDependencies)",
            cxxopts::value<std::string>())("g,git",
                                           "install recipe by git repo",
                                           cxxopts::value<std::string>())(
            ...
            "m,mirror", "configure github mirror",
            cxxopts::value<std::string>())("p,proxy", "configure git proxy",
                                           cxxopts::value<std::string>());
        ...
        if (result.count("verbose")) {
            ...
            SetConsoleColor(FOREGROUND_RED | FOREGROUND_GREEN |
                            FOREGROUND_BLUE);
#else
            printf("mirror: \033[32m%s\033[0m\nproxy: \033[32m%s\033[0m\ncache "
                   "directory: \033[32m%s\033[0m\nuser directory: "
                   "\033[32m%s\033[0m\n",
                   mirror.c_str(), proxy.c_str(), cache_dir.c_str(),
                   user_dir.c_str());
#endif
        }
            ...
            if (file_exist(cache_dir))
                for (const auto &entry :
                    ...
                    if (entry.is_directory() &&
                        entry.path().filename() != "." &&
                        entry.path().filename() != "..") {
                        std::cout << "delete directory: " << entry.path()
                                  << std::endl;
                        delete_directory(entry.path().string());
                    }
                }
        ...
        std::vector<spec::Recipe> recipes;
        if (file_exist(cache_dir + "/rppi/index.json")) {
            extract_recipes_from_rppi(cache_dir + "/rppi", recipes);
        } else
            goto updaterppi;
        if (!recipes.size()) {
            std::cout << "update rppi index" << std::endl;
            update_rppi(cache_dir + "/rppi", mirror, proxy);
            decltype(recipes)().swap(recipes);
            return 0;
        }
        ...
        if (result.count("install")) {
            ...
            std::vector<spec::Recipe> res = filter_recipes(recipes, repo, true);
            if (res.size()) {
            ...
            std::cout << "git install recipe by url : " << mirror << repo
                      << ".git";
            if (!recipe_file.empty())
                std::cout << ", with recipe file: " << recipe_file;
            std::cout << std::endl;
            ...
            std::vector<spec::Recipe> res =
                filter_recipes(recipes, repo, false);
            print_recipes(res);
            decltype(recipes)().swap(recipes);
            return 0;
            ...
            if (res.size()) {
                ...
                std::cout << "delete recipe by : "
                          << result["delete"].as<std::string>()
                          << " failed ||-_-" << std::endl;
            }
            decltype(recipes)().swap(recipes);
            return 0;
            ...
            if (res.size()) {
                ...
                std::cout << "purge recipe by : "
                          << result["purge"].as<std::string>()
                          << " failed ||-_-" << std::endl;
            }
            decltype(recipes)().swap(recipes);
            return 0;
