...
void from_json(const json &j, Recipe &r) {
    ...
    DEFINE_OPTIONAL_FIELD_FROM_JSON("reverseDependencies",
                                    r.reverse_dependencies, StrVec)
...
void extract_recipes_from_rppi(const std::string &root,
...
void print_recipes(const std::vector<spec::Recipe> &recipes) {
...
std::vector<spec::Recipe>
filter_recipes(const std::vector<spec::Recipe> &recipes,
